...
class Real_Widget_Best_Sellers extends WP_Widget {
	 ...
	 * @return void
	 */
	function Real_Widget_Best_Sellers() {
	...
	function widget( $args, $instance ) {
		...
		if ( $r->have_posts() ) {
			...
			while ( $r->have_posts() ) {
				...
				echo '<li class="row">
					<a href="' . get_permalink() . '" class="widgettag"></a>'
						 . ( has_post_thumbnail() ? get_the_post_thumbnail( $r->post->ID, 'shop_large' ) : woocommerce_placeholder_img( 'shop_large' ) ) . '<h4 class="widgettitle">' 
						 . get_the_title() . '</h4><div class="price">' 
